...
import re
import json
from config import PATHS, IUCN_API, CATEGORY_MEANINGS
...
def query_country_data(plant_name):
    # Helper functions for querying the IUCN API
    # Query IUCN API for plant distribution by country.
    encoded_name = urllib.parse.quote(plant_name)
    url = f"{IUCN_API['base_country_url']}{encoded_name}?token={IUCN_API['token']}"
...
def query_threats(plant_name):
    ...
    encoded_name = urllib.parse.quote(plant_name)
    url = f"{IUCN_API['base_threats_url']}{encoded_name}?token={IUCN_API['token']}"
    response = requests.get(url)
...
def query_conservation_status(plant_name):
    ...
    encoded_name = urllib.parse.quote(plant_name)
    url = f"{IUCN_API['base_species_url']}{encoded_name}?token={IUCN_API['token']}"
    response = requests.get(url)
...
def fetch_and_process_active_ingredients(cnx, cursor):
    ...
    # Fetch active ingredients from the database, clean the names, and process the data
    # using the IUCN API.
    
    print("Fetching active ingredients from the database...")
...
def iucn_conservation_status_PM(cnx, cursor):
    ...
    try:
        ...
        output_active_ingredients_path = f"{PATHS['output_int_dir']}/iucn_conservation_active_ing_data.json"
        output_dcb_path = f"{PATHS['output_int_dir']}/iucn_conservation_dcb_data.json"

        with open(output_active_ingredients_path, "w") as f:
            json.dump(output_active_ingredients, f, indent=4)

        with open(output_dcb_path, "w") as f:
    ...
    print("Finished IUCN Conservation Status processing.")
    return